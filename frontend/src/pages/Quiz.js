...
export default function Quiz() {
    ...
    const handleFinalSubmit = async (finalData) => {
        try {
            const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/assessment/submit`, {
    ...
    const handleNext = (val) => {
        ...
        const score = currentQ.isReverse ? (6 - val) : val;
    ...
    if (loading) {
        return (
            <div className="vh-100 bg-white d-flex align-items-center justify-content-center text-dark font-monospace">
                ...
                <span style={{ color: '#b690f1', fontWeight: 'bold' }}>{t('quizPage.loading')}</span>
            </div>
        );
    }
            ...
            <div className="min-vh-100 bg-white d-flex align-items-center justify-content-center py-5 px-3">
                <div className="container" style={{ maxWidth: '550px' }}>
                    ...
                    >
                        ...
                        <h1 className="display-1 fw-bold text-black mb-1" style={{ color: '#000', letterSpacing: '-5px' }}>
                        ...
                        <div className="mb-5 mt-2">
                            <span className="px-4 py-2 bg-black text-white rounded-pill fw-bold small shadow-sm">
                                {summaryReport.progress.improvement >= 0
                                    ? `+ ${summaryReport.progress.improvement}% ${t('quizPage.growth')}`
                                    : `- ${Math.abs(summaryReport.progress.improvement)}% ${t('quizPage.deviation')}`}
                            </span>
                        </div>

                        <div className="text-start p-4 rounded-4 mb-5" style={{ background: 'rgba(0,0,0,0.06)', border: '1px solid rgba(0,0,0,0.1)' }}>
                            <p className="small fw-bold mb-2 uppercase ls-wide" style={{ color: '#000', opacity: '0.6', fontSize: '0.65rem' }}>
                                {t('quizPage.feedback')}
                            </p>
                        ...
                        <div className="d-grid gap-3 px-2">
                            <button className="btn py-3 fw-bold text-white rounded-pill shadow-xl" onClick={() => navigate('/student-dashboard')} style={{ background: '#000', border: 'none', fontSize: '1rem' }}>
                                {t('quizPage.goToAnalysis')}
                            </button>
                            <button className="btn btn-link text-black text-decoration-none fw-bold small mt-1" onClick={() => window.location.reload()}>
                                {t('quizPage.retake')}
                            </button>
                        </div>
                    </div>
            ...
            <div className="container" style={{ maxWidth: '850px' }}>
                <div className="text-center mb-5 mt-4">
                    <p className="fw-bold small ls-widest text-uppercase" style={{ color: '#b690f1' }}>
                        {t('quizPage.scenarioOf', { scenario: sitIdx + 1, item: (sitIdx * 5) + quesIdx + 1 })}
                    </p>
                ...
                <div className="aras-card border-0 shadow-sm text-center py-5" style={{ background: '#ffffff', border: '1px solid #e4e4e7' }}>
                    {!isPaused ? (
                        ...
                        <div className="py-5 text-center">
                            <h4 className="text-dark mb-5 opacity-75">{t('quizPage.situationComplete')}</h4>
