...
export default function Roadmap() {
    ...
    useEffect(() => {
        const fetchRoadmap = async () => {
            ...
            try {
                const resRes = await axios.get(`${process.env.REACT_APP_API_URL}/api/assessment/user-result/${userId}`);
                const roadRes = await axios.post(`${process.env.REACT_APP_API_URL}/api/ai-support/generate-roadmap`, { scores: resRes.data });
    ...
    if (loading) {
        return (
            <div className="vh-100 bg-white d-flex flex-column align-items-center justify-content-center text-dark">
                ...
                <p className="fw-bold ls-widest mt-4 uppercase" style={{ color: '#9D59EF' }}>{t('roadmapPage.loading')}</p>
            </div>
        );
    }
        ...
        <div className="min-vh-100 bg-white text-dark py-5 px-3">
            <div className="container py-5 text-start">
                <div className="mb-5">
                    ...
                    <h1 className="display-4 fw-bold text-dark mb-2">{t('roadmapPage.title')}</h1>
                    <p style={{ color: '#71717a' }} className="fs-5">{t('roadmapPage.subtitle')}</p>
                </div>
                ...
                <div className="mx-auto mt-5" style={{ maxWidth: '800px' }}>
                    {steps.map((item, i) => (
                        <div key={i} className="d-flex mb-5 position-relative">
                            ...
                            <div className="ms-4 p-4 shadow-sm w-100" style={{ background: '#ffffff', borderRadius: '20px', border: '1px solid #e4e4e7' }}>
